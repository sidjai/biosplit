...
```{r}
bareas <- "C:/Users/Siddarta.Jairam/Documents/Documentation/"
...
regStats <- vapply(fullDir,function(f){
	vapply(areas, function(area){
		return(c(simAreaStats(f, paste0(bareas, area)),recursive=TRUE))
	},rep(1,3))
...
####Table `r figList$arrTable`. The first arrival for both populations in the specific regions. The spatial range is the region. The week of first occurance in any location in the spatial range is reported as the pair (FL first occurance, TX first occurance)
`r kable(arrFinal)`
